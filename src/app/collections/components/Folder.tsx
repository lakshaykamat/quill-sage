...
type ItemData = any;
const FolderCard = ({ name,id }: {name: string,id:string }) => {
  ...
  const handleItemClick = async({ id,props,}: ItemParams<ItemProps, ItemData>) => {
    switch (id) {
      case "del":
        // @ts-ignore
  ...
  return (
    <>
      ...
      <div id={id} key={id} onContextMenu={displayMenu}>
        <Link href={`collections/${name}`} className="flex flex-col items-center p-2 transition-all duration-150 ease-in-out rounded-lg delay-50 hover:bg-slate-100">
          <AiFillFolder className="w-12 h-12 text-slate-600 md:w-24 md:h-24" />
          <span className="font-medium">{name}</span>
        </Link>
      </div>
      <Menu id={MENU_ID}>
        <Item id="del" data={id} onClick={handleItemClick}>Delete</Item>
        <Item id="rename" data={id} onClick={handleItemClick}>Rename</Item>
      </Menu>
    </>
  );
};
