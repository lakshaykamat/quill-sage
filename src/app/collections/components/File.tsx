...
const File = ({noteid,noteTitle,name}:{name:string,noteid:string,noteTitle:string}) => {
  ...
  return (
    <Link href={`${name}/${noteid}`} title={noteTitle} className="flex flex-col items-center p-2 transition-all duration-150 ease-in-out rounded-lg delay-50 hover:bg-slate-200">
